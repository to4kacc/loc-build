...
# =========================================================
# WRT-CI 统一视觉引擎 (Ui.sh) - V6.5 Final Pro
# =========================================================
    ...
    case "$CURRENT_LANG" in
        "en")
            case "$key" in
                ...
                "build") echo "Launch Interactive Build Workflow" ;;
                "rebuild") echo "Rebuild Last Machine Settings" ;;
                "sync") echo "Sync Source Code and Assets" ;;
                "timer") echo "Automation & Scheduler Manager" ;;
                "pkg") echo "Extension Package Manager" ;;
                "env") echo "Init System Build Environment" ;;
                "lang") echo "Switch Language (中文)" ;;
                "exit") echo "Exit and Close Dashboard" ;;
                "mod_source") echo "Modify Remote Repository" ;;
                "mod_branch") echo "Modify Target Branch" ;;
                "manage_model") echo "Manage Build Machine List" ;;
                "config_cache") echo "Cache Retention Policy" ;;
                "save_exit") echo "Save Config and Return" ;;
                "discard") echo "Discard and Return" ;;
                "set_sched") echo "Setup Cron Schedule" ;;
                "check_task") echo "View Active Task Details" ;;
                "term_sched") echo "Terminate All Tasks" ;;
                "config_pipe") echo "Automated Pipeline Center" ;;
                "view_logs") echo "View Real-time Process Logs" ;;
                ...
                "build_msg") echo "Core engine is running..." ;;
                *) echo "$key" ;;
            esac ;;
        *) # 中文
            case "$key" in
...
msg_step() { local key="step$1"; echo -e "\n ${BP}--- STEP $1 : $(T $key) ---${NC}"; }
...
multi_select_menu() { 
    ...
    while true; do 
        echo -ne "  ${BOLD}${title}${NC} " >&2; 
        [ "$CURRENT_LANG" == "en" ] && echo -e "${W}(Space:Toggle, Enter:Confirm, q:Back)${NC}" >&2 || echo -e "${W}(空格:勾选, Enter:确定, q:返回)${NC}" >&2; 
        for i in "${!options[@]}"; do 
    ...
    tput cuu $((${#options[@]} + 1)) >&2; tput ed >&2; tput cnorm >&2; 
    local res=""; for i in "${!active[@]}"; do [ "${active[$i]}" -eq 1 ] && res+="$i "; done; RET_VAL="$res"; 
}
